...
for i=1:classernum
    ...
    ELM_Kernel_Model = elm_kernel_train(sample_train, sample_oob, 1, C, 'Mix', [rbf_para;poly_para;b_para]);
...
RF_Model{1,1}=improve_bad_model{8,1};
RF_Model{1,2}=improve_bad_model{9,1};
RF_Model{1,3}=improve_bad_model{10,1};
RF_Model{1,4}=improve_bad_model{11,1};
RF_Model{1,5}=improve_bad_model;
%% 循环优化剩余的分类器
acc_flag=improve_bad_model{11,1}; % 设置准确率标杆
wcfw=0.01; % 允许的误差范围
for i=2:classernum
    acc_tmp= RF_Model{i,4};
    if abs(acc_flag-acc_tmp) > wcfw
        bad_model=RF_Model{i,5};
        improve_train_data=bad_model{6,1};
        improve_test_data=bad_model{7,1};
        improve_bad_model=improveBase(improve_train_data,improve_test_data);
        RF_Model{i,1}=improve_bad_model{8,1};
        RF_Model{i,2}=improve_bad_model{9,1};
        RF_Model{i,3}=improve_bad_model{10,1};
        RF_Model{i,4}=improve_bad_model{11,1};
        RF_Model{i,5}=improve_bad_model;
    end
end
