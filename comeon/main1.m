...
for i=1:sizepop
    ...
    for k=1:3  %交叉验证k=5，5个包轮流作为测试集
        ...
        %[result(k,1), result(k,2), result(k,3), result(k,4)] = elm_kernel1(train_data, test_data, 1, C, 'Mix', [rbf_para;poly_para;b_para]);
        [result(k,1), result(k,2), result(k,3), result(k,4),TY,ELM_Kernel_Model] = elm_kernel_train(train_data, test_data, 1, C, 'Mix', [rbf_para;poly_para;b_para]);
    end
